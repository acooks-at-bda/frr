...
/* OSPF6_LSTYPE_E_ROUTER */
static char *ospf6_e_router_lsa_get_nbr_id(struct ospf6_lsa *lsa, char *buf,
					   int buflen, int pos)
{
	char buf1[INET_ADDRSTRLEN], buf2[INET_ADDRSTRLEN];
	struct tlv_router_link *tlv = nth_tlv(lsa->header, pos);

	if (!tlv || !buf || buflen < (2 + 2 * INET_ADDRSTRLEN))
		return NULL;

	inet_ntop(AF_INET, &tlv->neighbor_interface_id, buf1, sizeof(buf1));
	inet_ntop(AF_INET, &tlv->neighbor_router_id, buf2, sizeof(buf2));
	snprintf(buf, buflen, "%s/%s", buf2, buf1);
	return buf;
}
...
static void router_lsa_originate(struct ospf6_area *oa, int lstype)
{
	...
	if (IS_OSPF6_DEBUG_ORIGINATE(ROUTER)) {
		if (lstype == OSPF6_LSTYPE_ROUTER)
			...
			zlog_debug("Originate E-Router-LSA for Area %s", oa->name);
	}
...
static int cb_print_tlv_einp(void *desc, void *cb_data)
{
	struct tlv_intra_area_prefix *tlv_inp = desc;
	struct cbd_prefix_printer *cbd = cb_data;
	struct ospf6_prefix *prefix = (struct ospf6_prefix *)(tlv_inp + 1);
...
{
	struct tlv_ipv6_link_local_address *tlv = desc;
	struct cbd_prefix_printer *cbd = cb_data;
	struct in6_addr *addr = &tlv->addr;
	char buf[32];
			       ...
			       json_object *json_obj, bool use_json)
{
	...
	static const struct tlv_handler handlers[] = {
		...
		{ OSPF6_TLV_IPV6_LL_ADDR, cb_tlv_ip6lladdr_to_str },
		{ OSPF6_TLV_INTRA_AREA_PREFIX, cb_print_tlv_einp },
		{ 0 }
	};
...
static void link_lsa_originate(struct ospf6_interface *oi, int lstype)
{
	...
	if (IS_OSPF6_DEBUG_ORIGINATE(LINK)) {
		if (lstype == OSPF6_LSTYPE_LINK)
			...
			zlog_debug("Originate E-Link-LSA for Interface %s", oi->interface->name);
	}
	...
	if (lstype == OSPF6_LSTYPE_LINK) {
		...
		/* Fill the IPV6 Link-Local Address TLV */
		...
		/* Fill the Intra-Area Prefix TLVs... */
		tlv_op = (struct tlv_intra_area_prefix *)TLV_HDR_NEXT(
			&tlv_ip6ll->header);

		op = (struct ospf6_prefix *)(TLV_BODY(tlv_op) +
					     tlv_body_min_size(
						     OSPF6_TLV_INTRA_AREA_PREFIX));
		...
		for (route = ospf6_route_head(oi->route_connected); route;
			...
			tlv_op->header.length =
				htons((char *)OSPF6_PREFIX_NEXT(op) -
				      TLV_BODY(tlv_op));
			tlv_op = (struct tlv_intra_area_prefix *)TLV_HDR_NEXT(
				&tlv_op->header);

			op = (struct ospf6_prefix *)(TLV_BODY(tlv_op) +
				tlv_body_min_size(OSPF6_TLV_INTRA_AREA_PREFIX));
		}
...
/*
 * lstype is one of
 * OSPF6_LSTYPE_INTRA_PREFIX or
 * OSPF6_LSTYPE_E_INTRA_PREFIX
 */
static void intra_prefix_lsa_originate_stub(struct ospf6_area *oa, int lstype)
{
	char buffer[OSPF6_MAX_LSASIZE];
	struct ospf6_lsa_header *lsa_header;
	...
	char *lsdesc;
	struct ospf6_prefix *op;
	struct tlv_intra_area_prefix *tlv;
	...
	/* find previous LSA */
	old = ospf6_lsdb_lookup(htons(lstype), htonl(0), oa->ospf6->router_id,
				oa->lsdb);
	...
	if (!IS_AREA_ENABLED(oa)) {
		if (old) {
			ospf6_lsa_purge(old);
			/* find previous LSA */
			old_next = ospf6_lsdb_lookup(htons(lstype),
						     htonl(++ls_id),
						     oa->ospf6->router_id,
						     oa->lsdb);
			...
			while (old_next) {
				...
				old_next =
					ospf6_lsdb_lookup(htons(lstype),
							  htonl(++ls_id),
							  oa->ospf6->router_id,
							  oa->lsdb);
			}
	...
	if (IS_OSPF6_DEBUG_ORIGINATE(INTRA_PREFIX))
		zlog_debug("Originate %s for area %s's stub prefix",
			   (lstype == OSPF6_LSTYPE_INTRA_PREFIX)
	...
	if (lstype == OSPF6_LSTYPE_INTRA_PREFIX)
		intra_prefix_lsa->ref_type = htons(OSPF6_LSTYPE_ROUTER);
	else
		intra_prefix_lsa->ref_type = htons(OSPF6_LSTYPE_E_ROUTER);
	...
	prefix_num = 0;
	lsdesc = lsdesc_start_lsa_type(lsa_header, lstype);
	for (route = ospf6_route_head(route_advertise); route;
	     route = ospf6_route_best_next(route)) {
		if ((lsdesc - (char *)lsa_header) > MAX_LSA_PAYLOAD) {
			...
			lsa_header->seqnum =
				ospf6_new_ls_seqnum(lsa_header->type,
						    ...
						    lsa_header->adv_router,
						    oa->lsdb);
			lsa_header->length = htons(lsdesc - (char *)lsa_header);
			...
			/* Fill Intra-Area-Prefix-LSA */
			intra_prefix_lsa->ref_type = htons(lstype);
			intra_prefix_lsa->ref_id = htonl(0);
			intra_prefix_lsa->ref_adv_router = oa->ospf6->router_id;
		...
		if (lstype == OSPF6_LSTYPE_INTRA_PREFIX) {
			...
			lsdesc = (char *)OSPF6_PREFIX_NEXT(op);
		} else {
			/* lstype == OSPF6_LSTYPE_E_INTRA_PREFIX */
			tlv = (struct tlv_intra_area_prefix *)lsdesc;
			...
			tlv->header.type = htons(OSPF6_TLV_INTRA_AREA_PREFIX);
			tlv->header.length = htons(
				tlv_body_min_size(OSPF6_TLV_INTRA_AREA_PREFIX) +
				OSPF6_PREFIX_SIZE(op));

			tlv->metric = htonl(0x00FFFFFF & route->path.cost);

			lsdesc = (char *)TLV_HDR_NEXT((struct tlv_header *)tlv);
		}
		prefix_num++;
	}
	...
	if (lstype == OSPF6_LSTYPE_INTRA_PREFIX)
		...
		intra_prefix_lsa->prefix_num = htons(0);
	...
	for (old_route = old; old_route; old_route = old_route->next) {
		...
		for (ALL_LIST_ELEMENTS(old_route->paths, anode, anext, o_path)) {
			...
			if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
				zlog_debug("%s: route %pFX cost old %u new %u is not same, replace route",
			...
			if (listcount(old_route->paths)) {
				...
				if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
					zlog_debug("%s: route %pFX old cost %u new cost %u, delete old entry.",
	...
	for (old_route = old; old_route; old_route = old_route->next) {
		...
		if (old_route->path.cost == route->path.cost) {
			...
			if (o_path == NULL) {
				...
				if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
					zlog_debug("%s: route %pFX %p another path added with nh %u, effective paths %u nh %u",
						   __func__, &route->prefix, (void *)old_route,
						   listcount(ecmp_path->nh_list),
						   old_route->paths ? listcount(old_route->paths)
								    : 0,
			...
			for (ALL_LIST_ELEMENTS_RO(old_route->paths, anode,
				...
				if (ls_entry == NULL) {
					if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
						zlog_debug("%s: ls_prfix %pFX ls_entry not found.",
							   __func__, &o_path->ls_prefix);
					continue;
				}
			...
			if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
				zlog_debug("%s: route %pFX %p with final effective paths %u nh %u",
					   __func__, &route->prefix, (void *)old_route,
...
static int ospf6_intra_prefix_route_configure(struct ospf6_prefix *op, struct ospf6_lsa *lsa,
					      struct ospf6_route *ls_entry, struct prefix ls_prefix)
{
	struct interface *ifp = NULL;
	struct ospf6_path *path;
	struct ospf6_route *route, *old;
	char buf[PREFIX2STR_BUFFER];
	struct ospf6_area *area = OSPF6_AREA(lsa->lsdb->data);
	struct ospf6_intra_prefix_lsa *intra_prefix_lsa = lsa_after_header(lsa->header);

	/* Appendix A.4.1.1 */
	if (CHECK_FLAG(op->prefix_options, OSPF6_PREFIX_OPTION_NU)) {
		if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
			ospf6_linkstate_prefix2str((struct prefix *)OSPF6_PREFIX_BODY(op), buf,
						   sizeof(buf));
	...
	route = ospf6_route_create(area->ospf6);

	memset(&route->prefix, 0, sizeof(struct prefix));
	route->prefix.family = AF_INET6;
	route->prefix.prefixlen = op->prefix_length;
	ospf6_prefix_in6_addr(&route->prefix.u.prefix6, intra_prefix_lsa, op);
	...
	memcpy(&route->path.ls_prefix, &ls_prefix, sizeof(struct prefix));

	if (intra_prefix_lsa->ref_adv_router == area->ospf6->router_id) {
		/* the intra-prefix are directly connected */
		ifp = if_lookup_prefix(&route->prefix, area->ospf6->vrf_id);
	}
	...
	old = ospf6_route_lookup(&route->prefix, area->route_table);
	if (old) {
		if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
			prefix2str(&route->prefix, buf, sizeof(buf));
			zlog_debug("%s Update route: %s old cost %u new cost %u paths %u nh %u",
				   __func__, buf, old->path.cost, route->path.cost,
				   listcount(route->paths), listcount(route->nh_list));
		}
		ospf6_intra_prefix_route_ecmp_path(area, old, route);
	} else {
		if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
			prefix2str(&route->prefix, buf, sizeof(buf));
			zlog_debug("%s route %s add with cost %u paths %u nh %u", __func__, buf,
				   route->path.cost, listcount(route->paths),
				   listcount(route->nh_list));
		}
		ospf6_route_add(route, area->route_table);
	}
...
struct cbd_intra_prefix_add {
	struct ospf6_lsa *lsa;
	struct ospf6_route *ls_entry;
	struct prefix *ls_prefix;
};

static int cb_intra_prefix_add(void *desc, void *cb_data)
{
	struct ospf6_prefix *prefix = desc;
	struct cbd_intra_prefix_add *cbd = cb_data;
	struct ospf6_route *ls_entry;
	struct ospf6_lsa *lsa = cbd->lsa;
	struct prefix ls_prefix = *(cbd->ls_prefix);

	ls_entry = cbd->ls_entry;
	return ospf6_intra_prefix_route_configure(prefix, lsa, ls_entry, ls_prefix);
}

static int cb_e_intra_prefix_add(void *desc, void *cb_data)
{
	struct tlv_intra_area_prefix *tlv = desc;
	struct cbd_intra_prefix_add *cbd = cb_data;
	struct ospf6_prefix *prefix;
...
void ospf6_intra_prefix_lsa_add(struct ospf6_lsa *lsa)
{
	...
	char buf[PREFIX2STR_BUFFER];
	static const struct tlv_handler handlers[] = {
		{ OSPF6_TLV_RESERVED, cb_intra_prefix_add },
		{ OSPF6_TLV_INTRA_AREA_PREFIX, cb_e_intra_prefix_add },
		{ 0 }
	};
	struct cbd_intra_prefix_add cbd = { .lsa = lsa };
	...
	/* find the link state reference prefix */
	switch (ntohs(intra_prefix_lsa->ref_type)) {
	...
	/* find the route for the link state reference prefix */
	...
	cbd.ls_entry = ls_entry;
	cbd.ls_prefix = &ls_prefix;
						  ...
						  struct ospf6_route *route)
{
	...
	for (ALL_LIST_ELEMENTS(route->paths, anode, anext, o_path)) {
		...
		if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
			prefix2str(&route->prefix, buf, sizeof(buf));
			zlog_debug("%s: route %s path found with cost %u nh %u to remove.",
				   __func__, buf, o_path->cost, listcount(o_path->nh_list));
		}
	...
	if (nh_updated) {
		...
		if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
			prefix2str(&route->prefix, buf, sizeof(buf));
			zlog_debug("%s: route %s update paths %u nh %u", __func__, buf,
...
void ospf6_intra_route_calculation(struct ospf6_area *oa)
{
	...
	/* install E_INTRA_PREFIX's */
	type = htons(OSPF6_LSTYPE_E_INTRA_PREFIX);
	for (ALL_LSDB_TYPED(oa->lsdb, type, lsa))
		ospf6_intra_prefix_lsa_add(lsa);
	...
	for (route = ospf6_route_head(oa->route_table); route; route = nroute) {
		if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
			prefix2str(&route->prefix, buf, sizeof(buf));
			zlog_debug("%s: route %s, flag 0x%x", __func__, buf, route->flag);
		}
		...
		if (CHECK_FLAG(route->flag, OSPF6_ROUTE_REMOVE))
			...
			if (IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX))
				zlog_debug("%s: Originate summary for route %s", __func__, buf);
...
static void ospf6_brouter_debug_print(struct ospf6_route *brouter)
{
	...
	zlog_info("  memory: prev: %p this: %p next: %p parent rnode: %p", (void *)brouter->prev,
		  (void *)brouter, (void *)brouter->next, (void *)brouter->rnode);
	zlog_info("  type: %d prefix: %s installed: %s changed: %s", brouter->type, destination,
		  installed, changed);
	...
	zlog_info("  options: %s router-bits: %s metric-type: %d metric: %d/%d", options, capa,
		  brouter->path.metric_type, brouter->path.cost, brouter->path.u.cost_e2);
	zlog_info(" paths %u nh %u", listcount(brouter->paths), listcount(brouter->nh_list));
}
...
void ospf6_intra_brouter_calculation(struct ospf6_area *oa)
{
	...
	if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID(oa->area_id) || IS_OSPF6_DEBUG_ROUTE(MEMORY))
		zlog_debug("%s: border-router calculation for area %s", __func__, oa->name);
	...
	for (brouter = ospf6_route_head(oa->ospf6->brouter_table); brouter;
		...
		if (CHECK_FLAG(brouter->flag, OSPF6_ROUTE_REMOVE)) {
			if (IS_OSPF6_DEBUG_BROUTER ||
				...
				zlog_debug("%s: brouter %s disappears via area %s", __func__,
	...
	if (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID(oa->area_id) || IS_OSPF6_DEBUG_ROUTE(MEMORY))
		zlog_debug("%s: border-router calculation for area %s: done", __func__, oa->name);
}
...
static struct ospf6_lsa_handler e_router_handler = {
	...
	.lh_short_name = "ERtr",
	.lh_show = ospf6_router_lsa_show,
	.lh_get_prefix_str = ospf6_e_router_lsa_get_nbr_id,
	.lh_debug = 0};
